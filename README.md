...
2. 若Omni_Comm.exe打开后超过60秒，仍然在每次重启时报错，则：
   2.1 若每次报错信息完全相同，请检查Omni设备连接情况后，重启Omni并执行上述步骤1.
   2.2 若每次报错信息有所变化，重启Omni并执行上述步骤1.
   2.3 若观察到Omni_Comm.exe输出Omni末端位姿信号后报错并进入重启倒计时，请尝试按Omni的pairing按钮，若无效则重启Omni并执行上述步骤1.
4. slx中的仿真步长可调整（默认为0.01），但请勿超过0.1、小于0.002、或设置为动态步长
5. slx中支持增添其他模块，包括：
   5.1. 绝大部分不涉及IP网络通信的模块
   5.2. 大部分设计IP网络通信的模块（如TCP模块、UDP模块），但请勿占用127.0.0.1:10301
6. 暂不兼容ROS通信模块，对ROS通信模块的支持将在后续版本给出
